...
string CProcess::ProcStatus()
{
  string result;
  ReadSmallFile file("/proc/self/status");
  file.readToString(65536, &result);
  return result;
}

string CProcess::ProcStat()
{
  string result;
...
std::vector<pid_t> CProcess::ThreadPids()
{
...
{
  std::lock_guard<std::mutex> procLock(proc_mu_);
  std::lock_guard<std::mutex> dataLock(data_mu_);
  pid_ = -1;
  running_ = false;
  FreeArgs();
  ClosePipes();
