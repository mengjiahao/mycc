...
#ifndef MYCC_UTIL_SPALY_TREE_H_
#define MYCC_UTIL_SPALY_TREE_H_
...
public:
  ...
  {
    assert(!find(*aValue)); // Duplicate elements are not allowed
  ...
  {
    T *last = lookup(aValue);
    assert(last); // This tree must contain the element being removed
    assert(Comparator::compare(aValue, *last) == 0);
...
private:
  ...
  {
    assert(aCmp); // Nodes shouldn't be equal!

    T **parentPointer = (aCmp < 0) ? &aLast->mLeft : &aLast->mRight;
    assert(!*parentPointer);
    *parentPointer = aNew;
    aNew->mParent = aLast;
  ...
  T *checkCoherency(T *aNode, T *aMinimum)
  {
    ...
    if (aMinimum)
    {
      assert(Comparator::compare(*aMinimum, *aNode) < 0);
    }
    if (aNode->mLeft)
    {
      assert(aNode->mLeft->mParent == aNode);
      T *leftMaximum = checkCoherency(aNode->mLeft, aMinimum);
      assert(Comparator::compare(*leftMaximum, *aNode) < 0);
    }
    if (aNode->mRight)
    {
      assert(aNode->mRight->mParent == aNode);
