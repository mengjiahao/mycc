...
void *trie_add(struct trie *root, const char *key, void *value)
{
  ...
  if (key[i])
  {
    ...
    if (child)
    {
      return trie_add(child, key + root->len + 1, value);
    }
    else
    {
      child = trie_make_node(key + root->len + 1, value);
...
static int32_t common_trie_done_setup;
...
static void init_common_trie(void)
{
        ...
        for (p = common_list; p->dirname; p++)
                trie_add(&common_trie, p->dirname, p);
