...
#ifndef MYCC_UTIL_SKIPLIST_H_
#define MYCC_UTIL_SKIPLIST_H_
...
SkipList<Key, Comparator>::NewNode(const Key &key, int32_t height)
{
    ...
    FindGreaterOrEqual(const Key &key) const
{
  ...
  while (true)
  {
    ...
    int32_t cmp = (next == nullptr || next == last_bigger)
                      ? 1
      ...
      prev_height_(1)
{
  ...
  for (int32_t i = 0; i < kMaxHeight_; i++)
  {
...
void SkipList<Key, Comparator>::Insert(const Key &key)
{
      ...
      (prev_[0] == head_ || KeyIsAfterNode(key, prev_[0])))
  {
    ...
    for (int32_t i = 1; i < prev_height_; i++)
    {
      prev_[i] = prev_[0];
    }
  }
