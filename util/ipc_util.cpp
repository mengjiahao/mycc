...
int unix_tcp_open(char *ip, unsigned int port)
{
  ...
  if (sockfd < 0)
  {
    PRINT_ERROR("tcp socket creation error\n");
    return -1;
  }

  if (-1 == bind(sockfd, (struct sockaddr *)&c_addr, sizeof(c_addr)))
  {
    PRINT_ERROR("tcp socket bind error\n");
    return -1;
  }
  return sockfd;
}
...
int unix_tcp_listen(int sockfd)
{
  ...
  {
    PRINT_ERROR("tcp socket (%d) listen error\n", sockfd);
    return -1;
  }
  return 0;
}
...
int unix_tcp_connect(char *ip, unsigned int port)
{
  ...
  {
    PRINT_ERROR("tcp socket conncet %s:%d error\n", ip, port);
    close(sockfd);
    return -1;
  }
}
...
int unix_tcp_poll_accept(int sockfd)
{
  ...
  if (ufds[0].revents & POLLIN)
  {
    ...
    if (newsockfd != -1)
    {
      PRINT_INFO("tcp socket (%d) poll accepted connection from %s:%d\n",
...
retry:
  ...
  if (ufds[0].revents & POLLOUT)
  {
    ...
    if (ret < 0)
    {
      PRINT_ERROR("tcp socket (%d) poll send error, close socket\n", sockfd);
      close(sockfd);
      return -1;
    }
    sent += ret;
...
retry:
  ...
  if (ufds[0].revents & POLLIN)
  {
    if (--count <= 0)
    {
      PRINT_ERROR("tcp socket (%d) poll recv loop too many times, close socket\n", sockfd);
      close(sockfd);
      return -1;
    }
    ...
    if (ret < 0)
    {
      PRINT_ERROR("tcp socket (%d) poll recv error, close socket\n", sockfd);
      close(sockfd);
      return -1;
    }
    if (ret == 0)
    {
      PRINT_ERROR("tcp socket (%d) poll recv error, peer is shutted down, close socket\n", sockfd);
      close(sockfd);
      return -1;
    }
    sent += ret;
...
int unix_msgq_send_buf_by_id(int msgq_id, void *buf, unsigned int buf_len)
{
  int ret = -1;
  char *p = NULL;
  long *type = NULL;
...
int unix_msgq_recv_buf_by_id(int msgq_id, void *buf, unsigned int buf_len)
{
  int ret = -1;
