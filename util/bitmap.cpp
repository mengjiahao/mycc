...
static uint64_t FindFirstSet(uint32_t w)
{
...
uint64_t Bitmap::FirstUnset(uint64_t start) const
{
  if (start >= nbits_)
  {
    return nbits_;
  ...
  uint64_t mask = (1ull << (start % kBits)) - 1;
  const uint64_t nwords = NumWords(nbits_);
  for (uint64_t i = start / kBits; i < nwords; i++)
  {
    Word word = word_[i] | mask;
    mask = 0; // Only ignore bits in the first word we process.
    uint64_t r = FindFirstSet(~word);

    if (r)
    {
      uint64_t result = i * kBits + (r - 1);
      if (result > nbits_)
        result = nbits_;
      return result;
    }
